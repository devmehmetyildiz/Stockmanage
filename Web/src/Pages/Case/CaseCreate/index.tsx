import { useAddCaseMutation } from '@Api/Case'
import { CaseAddRequest } from '@Api/Case/type'
...
const CaseAppForm = createAppForm<CaseAddRequest>()
...
const CaseCreate: React.FC = () => {
    ...
    const methods = useForm<CaseAddRequest>({
        mode: 'onChange',
    })

    const { getValues, formState, trigger } = methods

    const [AddCase, { isLoading }] = useAddCaseMutation()
                ...
                AddCase(getValues())
                    ...
                    .then(() => {
                        Pushnotification({
                            ...
                            Description: t('Pages.Cases.Messages.AddSuccess')
                        })
    ...
    return <Pagewrapper isLoading={isLoading} direction='vertical' alignTop gap={4}>
        <Title
            ...
            AdditionalName={t('Pages.Cases.Page.CreateHeader')}
            PageUrl={Paths.Cases}
        />
        <FormProvider<CaseAddRequest> {...methods}>
            <Contentwrapper>
                <Form>
