...
const ApprovalrequestsRejected = () => {

    const { t } = useTranslation()

    const { data, isFetching } = useGetApprovalrequestsQuery({ isActive: 1, Isapproved: 0, Isrejected: 1 })
    const { data: users, isFetching: isUsersFetching } = useGetUsersListQuery({ isActive: 1 })

    const TableQuery = useGetTableMetaQuery({ Key: 'approvalrequestsrejected' })
    ...
    const columns: ColumnType<ApprovalrequestItem>[] = [
        ...
        { header: t('Pages.Approvalrequests.Columns.RejectTime'), accessorFn: row => dateCellhandler(row?.ApproveTime) },
        { header: t('Pages.Approvalrequests.Columns.RejectUserID'), accessorFn: row => userCellhandler(row?.ApproveUserID), cell: wrapper => loaderCellhandler(wrapper, isUsersFetching), isMobile: true },
...
export default ApprovalrequestsRejected