...
interface UserLeftWorkerProps {
    setRecord: React.Dispatch<React.SetStateAction<UserListItem | null>>
    setDeleteOpen: React.Dispatch<React.SetStateAction<boolean>>
}

const UserLeftWorker: React.FC<UserLeftWorkerProps> = ({ setDeleteOpen, setRecord }) => {

    const { t } = useTranslation()

    const { data, isFetching } = useGetUsersListQuery({ isActive: 1, Isworker: 1, Isworking: 0 })

    const TableQuery = useGetTableMetaQuery({ Key: 'userleftworker' })
    ...
    const columns: ColumnType<UserListItem>[] = [
        ...
        { header: t("Pages.Users.Columns.Workendtime"), accessorKey: 'Workendtime', accessorFn: row => dateCellhandler(row.Workstarttime) },
        { header: t("Pages.Users.Columns.Leftinfo"), accessorKey: 'Leftinfo' },
